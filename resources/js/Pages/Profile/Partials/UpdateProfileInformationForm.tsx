...
type PageProps = {
    auth: { user: { name: string; email: string; profile_photo_path?: string | null } };
    ...
    return (
        <section className={className}>
            ...
            <form onSubmit={submit} className="mt-6 space-y-6" encType="multipart/form-data">
                    ...
                    <div className="mt-2 flex items-center gap-x-3">
                        <div className="relative h-24 w-24 overflow-hidden rounded-full ring-2 ring-gray-200">
                            {photoPreview ? (
                                ...
                                <img src={`/storage/${user.profile_photo_path}`} alt="Foto de perfil actual" className="h-full w-full object-cover" />
                            ) : (
                                <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=random`} alt="Avatar por defecto" className="h-full w-full object-cover" />
                            )}
                        </div>
                ...
                {flash?.message && (
                    <div className="mt-4 p-4 rounded-md bg-green-50 text-green-700">{flash.message}</div>
                )}
                {flash?.error && (
                    <div className="mt-4 p-4 rounded-md bg-red-50 text-red-700">{flash.error}</div>
                )}
            </form>
        </section>
    );
