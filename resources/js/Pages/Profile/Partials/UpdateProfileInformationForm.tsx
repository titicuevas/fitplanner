...
export default function UpdateProfileInformationForm({ status, className = '' }: Props) {
            ...
            <form onSubmit={submit} className="mt-6 space-y-6" encType="multipart/form-data">
                <div>
                    ...
                    <input
                        ...
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100"
                    />
                ...
                <div>
                    ...
                    <div className="mt-2 flex items-center gap-x-3">
                        <div className="relative h-24 w-24 overflow-hidden rounded-full ring-2 ring-gray-200 dark:ring-gray-600">
                            <img
                                src={photoPreview ?? avatarUrl(user)}
                                alt="Foto de perfil"
                ...
                <div className="flex items-center gap-4">
                    <button
                        type="submit"
                        className="rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-offset-gray-800"
                        disabled={processing}
                    >
                        {processing ? (
                ...
                {flash?.message && (
                    <div className="mt-4 rounded-md bg-green-50 p-4 text-green-700 dark:bg-green-950/40 dark:text-green-300">{flash.message}</div>
                )}
                {flash?.error && (
                    <div className="mt-4 rounded-md bg-red-50 p-4 text-red-700 dark:bg-red-950/40 dark:text-red-300">{flash.error}</div>
                )}
            </form>
        </section>
    );
